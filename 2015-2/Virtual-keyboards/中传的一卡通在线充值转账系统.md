...
*让我们用代码来实现它，源码如下（原创，转载请注明！）：*
    ...
    int main()
    {
    	...
    	long long keyboard;
    	int count = 0;
    	char Char2Int[2];//中间数组
    	char Input[10];//存放wireshark抓取的加密后密码
    	char Output[10];//存放输出的用户密码
    	char Middle[10];//存放倒序后的密码
    	char Keyboard[20];//存放屏幕键盘数字排列
